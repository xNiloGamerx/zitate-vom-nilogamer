...
function handlePinInputModalConfirm(onConfirmFunc) {
  if (pwdInput.value === pwd) {
    pwdInput.value = '';
    hideTerminateModal();
    document.getElementById('terminate-confirm-btn').removeEventListener('click', handlePinInputModalConfirm);
    onConfirmFunc();
  }
}
...
function runPinInputModal(onConfirmFunc) {
  hideSettingsModal();
  showTerminateModal();

  // Confirm Terminate
  document.getElementById('terminate-confirm-btn').addEventListener('click', () => {
    handlePinInputModalConfirm(onConfirmFunc);
  });
}
